...
import "github.com/SysUtils/go-1c-odata-generator/shared"
		...
		for _, e := range source.Entities {
			if _, ok := types[translateType(e.Name)]; ok {
				...
				for _, n := range e.Navigations {
					types[translateType(associations[n.Relationship][n.FromRole])] = true
					types[translateType(associations[n.Relationship][n.ToRole])] = true
				}
			}
		}
